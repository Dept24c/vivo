...
(deftest test-empty-sub-map
  (let [vc (vivo/vivo-client)
    ...
    (is (thrown-with-msg?
         ...
         (vivo/subscribe-to-state! vc "test" bad-sub-map
                                           (constantly true))))))
       ...
       (is (= true
              (au/<? (vivo/<update-state! vc [{:path [:local :users]
                                              ...
                                              {:path [:local :user-id]
                                               :op :set
                                               :arg user-id}]))))
       (is (= expected (vivo/subscribe-to-state! vc "test" sub-map
                                                         update-fn)))))))
   ...
   (ca/go
     (let [vc (vivo/vivo-client)
       ...
       (au/<? (vivo/<update-state! vc [{:path [:local :user-id]
                                        :op :set
                                        :arg user-id}]))
       (is (= {'id user-id} (vivo/subscribe-to-state! vc "test" sub-map
                                                              update-fn)))))))
...
(deftest test-bad-path-root-in-sub-map
  (let [vc (vivo/vivo-client)
    ...
    (is (thrown-with-msg?
         #?(:clj ExceptionInfo :cljs js/Error)
         #"Paths must begin with "
         (vivo/subscribe-to-state! vc "test" sub-map
                                           (constantly nil))))))
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
         ...
         (is (= {'title orig-title}
                (vivo/subscribe-to-state! vc "test" sub-map update-fn)))
     ...
     (try
       (let [vc (vivo/vivo-client)
         ...
         (is (= true (au/<? (vivo/<update-state! vc [{:path [:local :books]
                                                      :op :set
                                                      :arg books}]))))
         (is (= expected (vivo/subscribe-to-state!
         ...
         (is (= true (au/<? (vivo/<update-state!
                             vc [{:path [:local :books]
                                 ...
                                 {:path [:local :my-book-ids]
                                  :op :set
                                  :arg my-book-ids}]))))
         (is (= expected1 (vivo/subscribe-to-state! vc "test" sub-map
                                                    update-fn)))
         (is (= true (au/<? (vivo/<update-state!
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
             ...
             sub-ret (vivo/subscribe-to-state! vc "test" sub-map
                                               update-fn)]
         (is (= true update-ret))
         (is (= expected
                (-> sub-ret
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
         ...
         (is (= expected (vivo/subscribe-to-state! vc "test" sub-map update-fn))))
       (catch #?(:clj Exception :cljs js/Error) e
         (is (= :unexpected e)))))))
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
             ...
             _ (is (= true ret))
             expected {'title nil}]
         (is (= expected (vivo/subscribe-to-state! vc "test" sub-map update-fn opts))))
       (catch #?(:clj Exception :cljs js/Error) e
         (is (= :unexpected e)))))))
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
             ...
             ret0 (vivo/subscribe-to-state! vc "test" sub-map
                                            update-fn)
             _ (is (= {'titles []} ret0))
   ...
   (ca/go
     (try
       (let [vc (vivo/vivo-client)
             ...
             update-fn #(ca/put! ch %)
             ret2 (vivo/subscribe-to-state! vc "test" sub-map update-fn
                                            (u/sym-map resolution-map))
